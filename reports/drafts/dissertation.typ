...
They find localisation at the Standard Metropolitan Statistical Area (SMSA), state, and national levels. However, these results depended on how well the three-digit classes proxied endogenous factors. This issue led #cite(<RN3>, form: "prose") to use the six-digit classes for matching controls to reassess these results, finding no evidence of intranational localisation. They argued that three-digit controls hid significant intra-class heterogeneity, to which #cite(<RN4>, form: "prose") commented that boundaries between six-digit classes were arbitrary. #cite(<RN7>, form: "prose") wrote another reply, but the issue remained unsettled.
...
Inventors will choose the location with the highest potential benefit, i.e., $sum_(i'!=i) G(d_j, d_j(i'))$. Then, they might choose to interact with other inventors. Inventors interact if and only if the benefits of interacting exceeds the costs; formally, $G(d_(j(i), j(i')))>=c$. Hence, there must be a maximum distance $rho eq.triple max{d: G(d)>=c}$ at which interactions happen.

An agglomeration cluster is a set of locations interconnected by inventor interactions. For example, if inventor A interacts with inventor B, which interacts with inventor C, these form a cluster even if A does not interact with C. The latter is implied by the distance between A and C being larger than $rho$. They also define inventor agglomeration as a normalised measure of the number of agglomeration clusters for a fixed number of inventors. The fewer clusters there are, the more agglomerated inventors will be.

From these assumptions, #cite(<RN11>, form: "prose") generate a series of predictions I describe informally. First, if $rho$ increases, inventors who could not previously interact will now be able to. These additional interactions might result in neighbouring clusters merging. An increase in $rho$ will, therefore, weakly increase agglomeration. Second, these clusters will be larger, so for fixed $N$, the density of inventors in a cluster decreases. Third, larger, less dense clusters imply that the average bilateral distance between inventors increases. 
...
I used data from the USPTO's PatentsView platform, which helps disseminate intellectual property data. It contains quarterly updated datasets created from raw patent information. The datasets undergo an entity (e.g., inventors, assignees, and locations) disambiguation process. Locations are then standardised with place names from the OSMNames project #footnote[The OSMNames database contains place names from OpenStreetMap and geographic information. It is available at #link("https://osmnames.org/").], which further simplifies the pre-processing required for the density estimator. Finally, the dataset contains information on five different patent classification systems, including the USPC and its successor, the Cooperative Patent Classification (CPC).
...
I chose utility patents and corporate assignees to match the sample construction choices of #cite(<RN1>, form: "prose") and #cite(<RN3>, form: "prose"). However, these papers and #cite(<RN2>, form: "prose") included patents granted between the mid-70s and 1990 or 2000, so my sample covers a different period. This fact would likely imply weaker localisation effects if we consider the increased importance of long-distance collaboration. For example, #cite(<RN39>, form: "prose") highlight the increase in cross-country inventor teams.

Another difference is that #cite(<RN1>, form: "prose") exclusively analysed assignees which are top corporations or universities, but they did not find any differences between groups. Hence, I only restricted the number of assignees. I did so to simplify my code and since multiple assignees could reflect complex commercial arrangements with inventors. For example, as noted by #cite(<RN1>, form: "prose"), an invention might be assigneed to the US and a foreign office of a same company, but the internalised flow of knowledge within the broader organisation might not reflect this legal distinction. This may or may not be reflected in the choice of assignee for other patents, introducing a potential source of bias.
